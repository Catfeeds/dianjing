<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:46:"themes/admin_simpleboot3/admin\match\edit.html";i:1535178564;s:75:"E:\xampp\htdocs\thinkcmf\public\themes\admin_simpleboot3\public\header.html";i:1534389699;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
	<div class="wrap">
		<ul class="nav nav-tabs">
			...
			<li class="active"><a>编辑比赛</a></li>
		</ul>
		<form method="post" class="form-horizontal js-ajax-form margin-top-20" action="<?php echo url('Match/editPost'); ?>">
			...
			<div class="form-group">
				...
				<div class="col-md-6 col-sm-10">
					<input class="form-control" type="text" name="match_code" value="<?php echo $data['match_code']; ?>" readonly>
				</div>
			</div>
				...
				<div class="col-md-6 col-sm-10">
					<select name="disc_id" class="form-control" >
						...
						<?php if(!(empty($disc) || (($disc instanceof \think\Collection || $disc instanceof \think\Paginator ) && $disc->isEmpty()))): if(is_array($disc) || $disc instanceof \think\Collection || $disc instanceof \think\Paginator): $i = 0; $__LIST__ = $disc;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
								<option value="<?php echo $vo['id']; ?>" <?php echo $vo['id']=$data['disc_id']?'selected':''; ?>>
								<?php echo $vo['name']; ?>
								</option>
				...
				<div class="col-md-6 col-sm-10">
					<select name="warA" class="form-control" >
						...
						<?php if(!(empty($team) || (($team instanceof \think\Collection || $team instanceof \think\Paginator ) && $team->isEmpty()))): if(is_array($team) || $team instanceof \think\Collection || $team instanceof \think\Paginator): $i = 0; $__LIST__ = $team;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
								<option value="<?php echo $vo['id']; ?>" <?php echo $vo['id']=$data['warA']?'selected':''; ?>>
								<?php echo $vo['name']; ?>
								</option>
							<?php endforeach; endif; else: echo "" ;endif; endif; ?>
					</select>
			...
			<div class="form-group">
				...
				<div class="col-md-6 col-sm-10">
					<select name="warB" class="form-control" >
						<option value="">请选择</option>
						<?php if(!(empty($team) || (($team instanceof \think\Collection || $team instanceof \think\Paginator ) && $team->isEmpty()))): if(is_array($team) || $team instanceof \think\Collection || $team instanceof \think\Paginator): $i = 0; $__LIST__ = $team;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
								<option value="<?php echo $vo['id']; ?>" <?php echo $vo['id']=$data['warB']?'selected':''; ?>>
			...
			<div class="form-group">
				...
				<div class="col-md-6 col-sm-10">
					<input type="text" name="startTime" class="form-control js-bootstrap-datetime" value="<?php echo date('Y-m-d H:i:s',$data['time']); ?>">
				</div>
			</div>
